function timedata(busdata){
  ...
  for (var t = startTime; t < endTime; t+=timeStep_p) {
    var currPasPair = {time:t}, passengerSum = 0;
      ...
      if (DatetoStamp(item[0].time) >= t) {
        ...
        item.passenger = 0;
          ...
          for (var i = tlStartIndex[index]; i < item.length-1; i++) {
            if((DatetoStamp(item[i+1].time) >= DatetoStamp(t)) && (DatetoStamp(t) > DatetoStamp(item[i].time)))
            {
  ...
  var x = d3.time.scale()
    .domain(d3.extent(passengerPair, function(d) {return d.time;}))
    .range([0, width])
    .clamp(true);

  var y = d3.scale.linear()
    .domain(d3.extent(passengerPair, function(d) {return d.passenger;}))
    .range([height, 0]);
  ...
  var line = d3.svg.line()
    ...
    .y(function(d) { return y(d.passenger); });
